...
func NewTraceProvider(logger log.Logger, m vm.Metricer, cfg vm.Config, prestateProvider types.PrestateProvider, asteriscPrestate string, localInputs utils.LocalGameInputs, dir string, gameDepth types.Depth, serverArgs vm.ServerArgs) *AsteriscTraceProvider {
	return &AsteriscTraceProvider{
		logger:           logger,
		dir:              dir,
		prestate:         asteriscPrestate,
		generator:        vm.NewExecutor(logger, m, cfg, asteriscPrestate, localInputs, serverArgs),
		gameDepth:        gameDepth,
...
func NewTraceProviderForTest(logger log.Logger, m vm.Metricer, cfg *config.Config, localInputs utils.LocalGameInputs, dir string, gameDepth types.Depth, serverArgs vm.ServerArgs) *AsteriscTraceProviderForTest {
	p := &AsteriscTraceProvider{
		logger:         logger,
		dir:            dir,
		prestate:       cfg.AsteriscAbsolutePreState,
		generator:      vm.NewExecutor(logger, m, cfg.Asterisc, cfg.AsteriscAbsolutePreState, localInputs, serverArgs),
		gameDepth:      gameDepth,
		preimageLoader: utils.NewPreimageLoader(kvstore.NewDiskKV(vm.PreimageDir(dir)).Get),
	}
