...
library SafeSigners {
        ...
        returns (uint8 v_, bytes32 r_, bytes32 s_)
    {
        assembly {
            ...
            // Here we are loading the last 32 bytes, including 31 bytes of
            // 's'. There is no 'mload8' to do this. 'byte' is not working due
            // to the Solidity parser, so we use 'and' instead.
